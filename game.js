...
const dom = {
  ...
  // Auto-aim 開關
  aimToggle: document.getElementById('aim-toggle'),
...
// 效能優化:PixelRatio 上限 1.5 (原本 2),4K 螢幕大幅省 GPU
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5));
renderer.shadowMap.enabled = true;
// 效能優化:BasicShadowMap 比 PCFSoft 快 4 倍,陰影略粗糙但可接受
...
sun.castShadow = true;
// 效能優化:Shadow map 2048 → 1024 (75% 記憶體節省)
...
function renderMapDecor(mapData) {
  ...
  // 效能優化:合併 colliders 為 1 個 mesh (從 N 個 draw call 降到 1 個)
  const wallGeos = [];
  const coverGeos = [];
  for (const c of (mapData.colliders || [])) {
    const geo = new THREE.BoxGeometry(c.w, c.h, c.d);
    geo.translate(c.x, c.y, c.z);
    if (c.type === 'cover') coverGeos.push(geo);
    else wallGeos.push(geo);
  }
  if (wallGeos.length) {
    const wallMerged = mergeGeometries(wallGeos, false);
    const wallMesh = new THREE.Mesh(wallMerged, wallMat);
  ...
  if (coverGeos.length) {
    const coverMerged = mergeGeometries(coverGeos, false);
    const coverMesh = new THREE.Mesh(coverMerged, coverMat);
  ...
  // 效能優化:合併 decor 為 1 個 mesh(同顏色,共用 material)
  const decorMatCache = {};
  for (const d of (mapData.decor || [])) {
    ...
    m.castShadow = false;  // 效能優化:裝飾不投射陰影
    m.receiveShadow = true;
    group.add(m);
  }
...
// 預建所有武器(切武器時 show/hide,不再 dispose + rebuild)
const _weaponCache = {};
let _currentWeapon = null;
function _buildWeaponInner(key) {
  ...
  if (w.isMelee) {
    ...
    const blade = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: w.color, roughness: 0.3, metalness: 0.9 })
    );
    blade.position.set(0, 0, -w.barrelLength / 2 - 0.05);
    grp.add(blade);
    const guard = new THREE.Mesh(
      new THREE.BoxGeometry(0.14, 0.06, 0.03),
  ...
  grp.position.set(0.28, -0.32, -0.55);
  grp.rotation.y = -0.05;
  grp.rotation.x = -0.02;
  return grp;
}
...
function buildWeapon(key) {
  if (!WEAPONS_VISUAL[key]) return;
  // 切武器:只 hide 舊的,show 新的(預建於 cache)
  if (!_weaponCache[key]) {
    _weaponCache[key] = _buildWeaponInner(key);
    camera.add(_weaponCache[key]);
  }
  for (const k in _weaponCache) {
    _weaponCache[k].visible = false;
  }
  _weaponCache[key].visible = true;
  _currentWeapon = key;
}
...
function makeBotMesh() {
  // 效能優化:Bot 不投射陰影(只接收 + 玩家投射),省 shadow pass
  const root = new THREE.Group();
  const torso = new THREE.Mesh(
    new THREE.BoxGeometry(0.7, 0.9, 0.4),
    new THREE.MeshStandardMaterial({ color: 0x4a3a28, roughness: 0.9 })
  );
  torso.position.y = 1.1; torso.castShadow = false; root.add(torso);
  const head = new THREE.Mesh(
    new THREE.BoxGeometry(0.4, 0.4, 0.4),
    new THREE.MeshStandardMaterial({ color: 0xd8b890, roughness: 0.7 })
  );
  head.position.y = 1.8; head.castShadow = false; root.add(head);
  const scarf = new THREE.Mesh(
    new THREE.BoxGeometry(0.42, 0.12, 0.42),
  ...
  const legMat = new THREE.MeshStandardMaterial({ color: 0x2a2018, roughness: 0.9 });
  const legL = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.9, 0.3), legMat);
  legL.position.set(-0.18, 0.45, 0); legL.castShadow = false; root.add(legL);
...
socket.on('match_start', (data) => {
  ...
  // TDM 模式:顯示「武器切換」按鈕 (PK 鎖手槍不需要)
  if (data.mode === 'tdm' && typeof isTouchDevice !== 'undefined' && isTouchDevice) {
    const w1 = document.getElementById('mobile-w1');
    const w2 = document.getElementById('mobile-w2');
  ...
  const objText = data.mode === 'pk'
    ? `PK 對決 — ${data.map?.name || ''} (${data.map?.subtitle || ''}) · 先達 ${data.maxKills} 殺切下一張 · 第 ${(data.roundInMap ?? 0) + 1}/${data.roundsPerMap} 場 · 地圖 ${(data.mapIndex ?? 0) + 1}/${data.mapTotal ?? 1}`
...
socket.on('state', (data) => {
  ...
  if (me) {
    ...
    const w1 = document.getElementById('mobile-w1');
    const w2 = document.getElementById('mobile-w2');
    if (w1) w1.classList.toggle('active', me.weapon === 'nova');
    if (w2) w2.classList.toggle('active', me.weapon === 'ak47');
...
socket.on('match_end', (data) => {
  ...
  // 隱藏武器按鈕 (TDM 模式)
  const w1End = document.getElementById('mobile-w1');
  const w2End = document.getElementById('mobile-w2');
  if (w1End) w1End.style.display = 'none';
  if (w2End) w2End.style.display = 'none';
  if (isTouchDevice && typeof setMobileControlsVisible === 'function') setMobileControlsVisible(false);
...
dom.btnCreatePk?.addEventListener('click', () => {
  ...
  const name = dom.playerName.value.trim() || 'Player';
  // 預設 -1 = 隨機 (使用者沒點就隨機)
  const mapIndex = (typeof state._pickedMapIdx === 'number') ? state._pickedMapIdx : -1;
...
// Tab 鍵開啟換地圖面板,Esc 關閉 (M 鍵保留給靜音切換)
document.addEventListener('keydown', (e) => {
  if (e.code === 'Tab' && state.phase === 'playing' && state.mode === 'pk') {
    e.preventDefault();
...
// Auto-aim 開關 (Lobby checkbox)
dom.aimToggle?.addEventListener('change', () => {
  state.autoAim = dom.aimToggle.checked;
  sfx.click();
  if (state.autoAim) {
    console.log('🎯 Auto-aim: ON — 準心會自動對準最近敵人');
  } else {
    console.log('🎯 Auto-aim: OFF');
    if (dom.crosshair) dom.crosshair.classList.remove('locked');
  }
});
...
// =====================================================
// 玩家控制 (桌面 PointerLock + 手機觸控)
// =====================================================
const PITCH_LIMIT = Math.PI / 2 - 0.01;
const controls = new PointerLockControls(camera, document.body);
const isTouchDevice = ('ontouchstart' in window) || navigator.maxTouchPoints > 0 || location.search.includes('force_touch');
let mouseDown = false;
...
const mobileLookZone = document.getElementById('mobile-look-zone');
...
function setMobileControlsVisible(show) {
  if (mobileControls) mobileControls.classList.toggle('active', isTouchDevice && show);
}
...
if (isTouchDevice) {
  ...
  let lookX = 0;
  let lookY = 0;
  let lookStartX = 0;  // touch 開始位置 (用於絕對 delta 模式)
  let lookStartY = 0;

  const resetStick = () => {
    state.keys.w = state.keys.a = state.keys.s = state.keys.d = false;
    if (mobileStickKnob) mobileStickKnob.style.transform = 'translate(-50%, -50%)';
  };

  mobileStick?.addEventListener('touchstart', (e) => {
    e.preventDefault();
    if (moveTouchId !== null) return;
    moveTouchId = e.changedTouches[0].identifier;
  }, { passive: false });
  mobileStick?.addEventListener('touchmove', (e) => {
    e.preventDefault();
    const t = Array.from(e.changedTouches).find(x => x.identifier === moveTouchId);
    if (!t) return;
    const r = mobileStick.getBoundingClientRect();
    ...
    const dead = max * 0.25;
    state.keys.a = dx < -dead;
    state.keys.d = dx > dead;
    state.keys.w = dy < -dead;
    state.keys.s = dy > dead;
  }, { passive: false });
  const finishMove = (e) => {
    if (Array.from(e.changedTouches).some(x => x.identifier === moveTouchId)) {
      moveTouchId = null;
      resetStick();
    }
  };
  mobileStick?.addEventListener('touchend', finishMove, { passive: false });
  mobileStick?.addEventListener('touchcancel', finishMove, { passive: false });

  // 手機版視角拖曳:用絕對 delta (從 touch 開始到現在),靈敏度提高
  // 並在 touchend 時把目前位置設為新起點,讓連續拖曳可以無限制累積
  mobileLookZone?.addEventListener('touchstart', (e) => {
    e.preventDefault();
  ...
  mobileLookZone?.addEventListener('touchmove', (e) => {
    e.preventDefault();
    const t = Array.from(e.changedTouches).find(x => x.identifier === lookTouchId);
    if (!t) return;
    // 用「上次位置 → 現在位置」的增量 delta
    // 但在 touchend 時把目前位置設為新 start,讓使用者可以「持續拖」累積
    const sensitivity = 0.012;  // 3x,讓單次滑動可達 270°
    state.yaw -= (t.clientX - lookX) * sensitivity;
    state.pitch -= (t.clientY - lookY) * sensitivity;
    state.pitch = Math.max(-PITCH_LIMIT, Math.min(PITCH_LIMIT, state.pitch));
    // 把 yaw 限制在 [-π, π] 範圍內 (視角不會「卡」在 360° 邊界)
  ...
  const finishLook = (e) => {
    if (Array.from(e.changedTouches).some(x => x.identifier === lookTouchId)) {
      lookTouchId = null;
      // 不重置 lookX — 下次 touchstart 會設新位置
    }
  };
  mobileLookZone?.addEventListener('touchend', finishLook, { passive: false });
  mobileLookZone?.addEventListener('touchcancel', finishLook, { passive: false });

  const setFire = (value, e) => {
    e?.preventDefault();
    mouseDown = value;
    if (value) { sfx.init(); sfx.resume(); }
  };
  mobileFire?.addEventListener('touchstart', (e) => setFire(true, e), { passive: false });
  mobileFire?.addEventListener('touchend', (e) => setFire(false, e), { passive: false });
  mobileFire?.addEventListener('touchcancel', (e) => setFire(false, e), { passive: false });
  mobileReload?.addEventListener('touchstart', (e) => {
    e.preventDefault();
    state.reloadPressed = true;
    sfx.init(); sfx.resume(); sfx.reload();
  }, { passive: false });

  // 武器切換按鈕 (TDM 模式)
  const mobileW1 = document.getElementById('mobile-w1');
  const mobileW2 = document.getElementById('mobile-w2');
  const setWeapon = (w, e) => {
    e?.preventDefault();
    state.weaponSwitchTo = w;
    sfx.init(); sfx.resume(); sfx.click();
  };
  mobileW1?.addEventListener('touchstart', (e) => setWeapon('nova', e), { passive: false });
  mobileW2?.addEventListener('touchstart', (e) => setWeapon('ak47', e), { passive: false });
}
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.code === 'Digit3') state.weaponSwitchTo = 'knife';
...
function syncRemotePlayers(serverPlayers) {
  ...
  for (const p of serverPlayers) {
    ...
    if (!rp) {
      ...
      rp = { mesh, lastPos: null, walkPhase: 0, alive: true, x: p.x, y: p.y, z: p.z };
    ...
    rp.x = p.x; rp.y = p.y; rp.z = p.z; rp.alive = p.alive;
...
// 武器特效
// =====================================================
// Muzzle Flash 物件池 — 預建 8 個 PointLight,重用避免 GC
// =====================================================
let weaponKick = 0;
const MUZZLE_POOL_SIZE = 8;
const muzzlePool = [];
for (let i = 0; i < MUZZLE_POOL_SIZE; i++) {
  const light = new THREE.PointLight(0xffcc44, 0, 8);  // intensity 0 (關)
  light.visible = false;
  scene.add(light);
  muzzlePool.push({ light, inUse: false, expireAt: 0 });
}
let muzzlePoolIdx = 0;
...
function spawnMuzzleFlash(parentGroup, weaponKey) {
  ...
  let slot = null;
  for (let i = 0; i < MUZZLE_POOL_SIZE; i++) {
    const idx = (muzzlePoolIdx + i) % MUZZLE_POOL_SIZE;
    const p = muzzlePool[idx];
    if (!p.inUse || p.expireAt < now) {
      slot = p;
      muzzlePoolIdx = (idx + 1) % MUZZLE_POOL_SIZE;
      break;
    }
  }
  if (!slot) return;  // 池滿了,丟掉這次
  slot.inUse = true;
  slot.expireAt = now + 50;
  const offset = new THREE.Vector3(0.3, 1.1, -0.5);
  offset.applyEuler(parentGroup.rotation);
  slot.light.position.copy(parentGroup.position).add(offset);
  slot.light.intensity = 3;
  slot.light.visible = true;
}

// 每幀檢查過期(不依賴 setTimeout,改由 animate() 統一回收)
function updateMuzzlePool() {
  const now = performance.now();
  for (const p of muzzlePool) {
    if (p.inUse && p.expireAt < now) {
      p.inUse = false;
      p.light.intensity = 0;
...
// HUD / UI 更新
// =====================================================
// HUD 更新 — skip-if-same 避免每幀重設 DOM
// =====================================================
const _hudCache = { health: -1, danger: false, mag: -1, reserve: -1, weapon: '' };
function updateHUD() {
  // Health (只在變化時更新)
  const h = Math.ceil(state.health);
  if (h !== _hudCache.health) {
    dom.health.textContent = h;
    _hudCache.health = h;
  }
  if (state.health <= 30 !== _hudCache.danger) {
    dom.health.classList.toggle('danger', state.health <= 30);
    _hudCache.danger = state.health <= 30;
  }

  const ammo = state.ammo[state.weapon] || { mag: 0, reserve: 0 };
  // Mag (變化時觸發 bump 動畫)
  if (ammo.mag !== _hudCache.mag) {
    dom.ammoCur.textContent = ammo.mag;
    dom.ammoCur.classList.remove('bump');
    void dom.ammoCur.offsetWidth;  // reflow
    dom.ammoCur.classList.add('bump');
    _hudCache.mag = ammo.mag;
  }
  if (ammo.reserve !== _hudCache.reserve) {
    dom.ammoRes.textContent = ammo.reserve;
    _hudCache.reserve = ammo.reserve;
  }
  // Low ammo 樣式
  const isLow = ammo.mag <= 3;
  if (dom.ammoCur.classList.contains('low') !== isLow) {
    dom.ammoCur.classList.toggle('low', isLow);
  }
  // Weapon name
  const wn = WEAPONS_VISUAL[state.weapon]?.name || '—';
  if (wn !== _hudCache.weapon) {
    dom.weaponName.textContent = wn;
    _hudCache.weapon = wn;
  }
}

// =====================================================
// Match UI 更新 — skip-if-same 避免每幀重設 DOM
// =====================================================
const _matchCache = { scoreT: -1, scoreCT: -1, timer: '', mapName: '', mapSub: '', mapCur: -1, mapTotal: '', mode: '' };
function updateMatchUI() {
  if (!state.room) return;
  const scores = state.room.scores || { T: 0, CT: 0 };
  if (scores.T !== _matchCache.scoreT) {
    if (dom.scoreT) {
    ...
    _matchCache.scoreT = scores.T;
  }
  if (scores.CT !== _matchCache.scoreCT) {
    if (dom.scoreCT) {
    ...
    _matchCache.scoreCT = scores.CT;
  }
  // Timer (每秒才會變,通常)
  if (dom.matchTimer && state.matchStartedAt && state.phase === 'playing') {
    const sec = Math.floor((Date.now() - state.matchStartedAt) / 1000);
    const timerStr = `${String(Math.floor(sec / 60)).padStart(2, '0')}:${String(sec % 60).padStart(2, '0')}`;
    if (timerStr !== _matchCache.timer) {
      dom.matchTimer.textContent = timerStr;
      _matchCache.timer = timerStr;
    }
  }
  // PK 模式切換顯示
  if (state.room.mode !== _matchCache.mode) {
    _matchCache.mode = state.room.mode;
  ...
  if (state.room.mode === 'pk') {
    if (state.room.mapName !== _matchCache.mapName) {
      if (dom.mapName) dom.mapName.textContent = state.room.mapName || '';
      _matchCache.mapName = state.room.mapName || '';
    }
    if (state.room.mapSubtitle !== _matchCache.mapSub) {
      if (dom.mapSubtitle) dom.mapSubtitle.textContent = state.room.mapSubtitle || '';
      _matchCache.mapSub = state.room.mapSubtitle || '';
    }
    const roundInMap = (state.room.roundInMap ?? 0) + 1;
    if (roundInMap !== _matchCache.mapCur) {
      if (dom.mapCurrent) dom.mapCurrent.textContent = roundInMap;
      _matchCache.mapCur = roundInMap;
    }
    const total = state.room.mapTotal || 5;
    const idx = (state.room.mapIndex ?? 0) + 1;
    const totalStr = `${idx}/${total}`;
    if (totalStr !== _matchCache.mapTotal) {
      if (dom.mapTotal) dom.mapTotal.textContent = totalStr;
      _matchCache.mapTotal = totalStr;
    }
  }
}
...
// 準心自動瞄準 (找最近的敵人,平滑跟隨)
// =====================================================
function updateAutoAim(dt) {
  if (!state.autoAim || state.phase !== 'playing' || !state.alive) {
    if (dom.crosshair) dom.crosshair.classList.remove('locked');
    return;
  }
  const myPos = new THREE.Vector3().copy(camera.position);
  // 取得所有敵人 (遠端玩家 + bot,不含自己)
  const enemies = [];
  for (const [, rp] of state.remotePlayers) {
    if (rp?.mesh && rp.alive) {
      enemies.push({ x: rp.x, y: rp.y, z: rp.z, h: 1.1 });
    }
  }
  for (const [, b] of state.bots) {
    if (b?.mesh?.root && b.wasAlive !== false) {
      enemies.push({ x: b.mesh.root.position.x, y: 0, z: b.mesh.root.position.z, h: 1.1 });
    }
  }
  if (enemies.length === 0) {
    if (dom.crosshair) dom.crosshair.classList.remove('locked');
  ...
  const MAX_RANGE = 60;
  const FOV_DOT = 0.5;  // cos(60°) — ±60° 視野內算「視野內」
  for (const e of enemies) {
    const to = new THREE.Vector3(e.x - myPos.x, (e.y + e.h) - (myPos.y - 1.6), e.z - myPos.z);
    const dist = to.length();
    if (dist > MAX_RANGE) continue;
    to.normalize();
    const dot = to.dot(fwd);
    if (dot < FOV_DOT) continue;
    // 分數 = dot * 1000 - distance (越正面越近越好)
  ...
  // 計算目標 yaw / pitch (指向敵人胸口)
  const dx = bestTarget.x - myPos.x;
  const dy = (bestTarget.y + bestTarget.h) - (myPos.y - 1.6);
  const dz = bestTarget.z - myPos.z;
  const targetYaw = Math.atan2(dx, -dz);
  const horizDist = Math.sqrt(dx * dx + dz * dz);
  const targetPitch = Math.atan2(dy, horizDist);
  // 平滑跟隨 (lerp factor 看 dt)
  const lerpK = Math.min(1, 1 - Math.exp(-dt * 8));  // ~8/sec
  // 處理 yaw wrap (取最近路徑)
  let dyaw = targetYaw - state.yaw;
  if (dyaw > Math.PI) dyaw -= 2 * Math.PI;
  if (dyaw < -Math.PI) dyaw += 2 * Math.PI;
  state.yaw += dyaw * lerpK;
  state.pitch += (targetPitch - state.pitch) * lerpK;
  state.pitch = Math.max(-PITCH_LIMIT, Math.min(PITCH_LIMIT, state.pitch));
  if (dom.crosshair) dom.crosshair.classList.add('locked');
}
...
function animate() {
  // FPS 計數 (module-scope via window 確保可訪問)
  window._fpsCount = (window._fpsCount || 0) + 1;
  const now = performance.now();
  if (now - (window._fpsLast || 0) >= 1000) {
    if (document.getElementById('fps')) {
      document.getElementById('fps').textContent = window._fpsCount + ' FPS';
    }
    window._fpsCount = 0;
    window._fpsLast = now;
  }
  ...
  // 送輸入到 server (30Hz,匹配 server 處理頻率)
  inputSendTimer += dt;
  if (inputSendTimer >= 1 / 30) {
    inputSendTimer = 0;
...
// =====================================================
buildWeapon('nova');
buildWeapon('knife');             // 預建刀
_weaponCache['knife'].visible = false;  // 預設不顯示
_weaponCache['nova'].visible = true;    // 預設顯示 Nova
showLobby();
showLobbySection('connecting');
animate();

// 暴露給 console debug
window.__game = { state, scene, camera, socket, sfx, WEAPONS_VISUAL, animate };
